...
class Shopkeeper {
    ...
    async buy(userId, itemId){
        ...
        if(await this.inventory.hasItem(userId, itemId)) //check first if the user already has the item
            throw new ShopkeeperError('You already have this item!');
        
        if(userBankAccount.amount < boughtItem.price) // if not, make sure that the user has enough money
            throw new ShopkeeperError('You do not have enough money for this item!');
        
        const newUserBankAccount = new BankAccount(userId, userBankAccount.amount - boughtItem.price); // the bank account after paying for the item
        ...
        await this.transactionLogger.logToFile(userId, await this.shopRepo.read(itemId), true); // log the transaction
    }
    ...
    async sell(userId, itemId){
        ...
        const newUserBankAccount = new BankAccount(userId, userBankAccount.amount + sellIndex * soldItem.price); // the bank account after selling the item
        await this.inventory.remove(userId, itemId); // add the item to the inventory of the user
        await this.bankRepo.update(newUserBankAccount); // update the new tem account
        await this.transactionLogger.logToFile(userId, await this.shopRepo.read(itemId), false); // log the transaction
    }
}
